...
export function DebugPanel() {
  ...
  const runDiagnostics = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ error: "Failed to run diagnostics" }))
        throw new Error(errorData.message || errorData.error || `Failed to run diagnostics: ${response.status}`)
      }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        {title.includes("Pinecone") && data.config && (
          <CardDescription className="mt-2">
            <div className="text-xs text-muted-foreground">
              <div>Host: {data.config.host}</div>
              <div>Index: {data.config.indexName}</div>
              <div>API Key: {data.config.apiKeySet ? "Set" : "Not set"}</div>
            </div>
          </CardDescription>
        )}

        {title.includes("OpenAI") && status === "success" && (
          <CardDescription className="mt-2">
            <div className="text-xs text-muted-foreground">
              <div>Model: {data.model}</div>
              <div>Dimensions: {data.dimensions}</div>
              <div>Expected: {data.expectedDimensions}</div>
            </div>
          </CardDescription>
        )}
      </CardHeader>
